...
function renderButtons() {
  ...
  document.querySelectorAll('button.btn').forEach((btn) => {
    const theIngredient = btn.dataset.theIngredient;
    if(state[theIngredient]) {
...
function renderPrice() {
  ...
  let totalPrice = 10;   //base price inicial = 10 ou  let totalPrice = basePrice;

  let listIngredientPrice = '';     //começa cada li vazia, vai inserir cada ingrediente e seu preço

  for (let oneIngredient in ingredients) {
    if(state[oneIngredient]){
      listIngredientPrice += `<li>$ ${ingredients[oneIngredient].price} ${ingredients[oneIngredient].name}</li>`
